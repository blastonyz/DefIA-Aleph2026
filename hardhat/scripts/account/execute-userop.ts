...
async function main() {
    ...
    if (PAYMASTER_ADDRESS && USE_PAYMASTER) {
        ...
        if (paymasterDeposit === 0n) {
            throw new Error(
                "AA31: paymaster deposit is 0. Run npm run deposit:account so it funds both the smart account and the paymaster."
            );
        }
    }
    ...
    // Build execution target based on MOLTBOT_STRATEGY env var
